     ...
     <footer style="background-color: #536D37" class="max-h-300px w-full text-white py-6 flex flex-wrap ">
         ...
         <div class=" ml-4 flex flex-col mt-16">
             ...
             <div class="flex items-center mb-2">
                 <div
                     class="w-8 h-8 bg-custom-beige bg-opacity-65 rounded-md text-white text-base flex items-center justify-center mr-2">
         ...
         <div class="flex flex-col ml-20 mt-16">
             ...
             <div class="flex flex-row">
                 <a href="https://www.facebook.com"
                     class=" w-8 h-8 bg-white bg-opacity-30 rounded-md text-white text-xl flex items-center justify-center mr-2">
                     <i class="fa-brands fa-facebook-f"></i>
                 </a>
                 <a href="https://www.instagram.com"
                     class="w-8 h-8 bg-white bg-opacity-30 rounded-md text-white text-xl flex items-center justify-center ">
         ...
         {{-- Newsletter --}}
         <form method="post" action="{{ route('ajouterNewsletter') }}"
             class="flex-1 flex flex-col items-center text-center ml-16 mt-16">
             @csrf
             ...
             <input type="email" name="email"
                 class=" bg-opacity-40 bg-white px-4 py-2  mb-4 border-none rounded-md h-8 w-72 text-center placeholder-white focus:border-custom-marron focus:ring-custom-marron"
                 placeholder="{{ __('content.email') }}" required />
             <button style="background-color: #C4AA84"
                 class="text-white font-bold text-base px-6 py-1 rounded-md">{{ __('content.inscrire') }}</button>
             @if (session('success'))
                 <div class="font-bold">
                     {{ session('success') }}
                 </div>
