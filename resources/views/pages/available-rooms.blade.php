...
@section('navbar')
    <div class="fixed z-10 w-full">
        <a href="{{ route('menu') }}"
            class="absolute top-8 left-8 bg-gray-800 bg-opacity-65 text-white py-2.5 px-3 border-none rounded-md w-12 text-base inline-block text-center"><i
        ...
        @if (Auth::check())
            <div class="hidden sm:flex sm:items-center sm:ms-6 absolute top-9 right-24">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-bold rounded-md bg-custom-marron bg-opacity-90 text-white focus:outline-none transition ease-in-out duration-150">
        ...
        @else
            <a href="{{ route('login') }}"
                class="absolute top-8 right-24 bg-gray-800 bg-opacity-65 text-white py-2.5 px-3 font-bold border-none inline-block text-center rounded w-12 tracking-wide text-base">
        ...
        <div class="flex flex-row p-6 mt-12">
            <div class="flex-grow">
                @foreach ($availableCabanes as $cabane)
                    <div class="max-w-4xl border-2 border-custom-marron rounded-lg bg-white mb-6">
                        <div class="flex">
                            <div>
                                <img class="max-w-sm rounded" src="{{ Storage::url('images/cabaneeden.jpg') }}"
                                    alt="">
                            </div>
                            ...
                            <div class="flex flex-col p-8 justify-center">
                                <div class="flex flex-col space-y-4">
                                    ...
                                    <div class="max-w-sm text-custom-marron text-justify text-sm">
                                        {{ $cabane->description }} </div>
                                </div>

                                <div class="flex mt-12 justify-between items-center">
                                    <div class="flex flex-col">
                                        <div class="text-custom-marron"> Pour {{ $nombreNuitees }} nuit(s) </div>
                                        <div class="font-black text-2xl text-custom-marron "> {{ $cabane->prixTotal }} €
                                        </div>
                                    ...
                                    <form action="{{ route('extras') }}" method="POST">
                                        ...
                                        <button
                                            class="bg-custom-marron text-white font-bold rounded py-2 px-4 text-center uppercase"
            ...
            <div>
                <div class="sticky top-24  border-2 border-custom-marron rounded-lg p-6 ml-6" style="min-width: 300px;">
