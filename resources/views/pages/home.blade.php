...
@section('navbar')
    <div class="fixed z-10 w-full">
        <a href="{{ route('menu') }}"
            class="absolute top-8 left-8 bg-custom-vert bg-opacity-90 text-white py-2.5 px-3 border-none rounded-md w-12 text-base inline-block text-center"><i
        ...
        @else
            <a href="{{ route('login') }}"
                class="absolute top-8 right-52 bg-gray-400 bg-opacity-65 text-white py-2.5 px-3 font-bold border-none inline-block text-center rounded w-12 tracking-wide text-base">
                <i class="fa-solid fa-user"></i>
            </a>
        @endif
        ...
        <a href="{{ route('reserver') }}"
            class="absolute top-8 right-6 bg-custom-vert bg-opacity-90 tracking-widest text-white py-3 px-3 border-none rounded w-30 font-semibold text-sm uppercase">
            {{ __('content.reserver') }} </a>
    </div>
@endsection
...
@section('main')
    ...
    <div class=" w-full h-screen bg-fixed bg-center bg-cover" alt="Cabane en bois"
        style="background-image: url('{{ Storage::url('images/imageparallax.png') }}');"></div>
    ...
    <div class="bg-custom-vert h-screen w-full">
        ...
        <div class="justify-items-center grid grid-cols-2 gap-y-6 relative top-6">
            ...
            <a href="{{ route('cabane1') }}" class="relative group block overflow-visible ml-32">
                ...
                <img class="h-[200px] w-[250px] object-cover transition-opacity duration-300 ease-in-out group-hover:opacity-60"
                    src="{{ Storage::url('images/nid.png') }}" alt="Cabane Nid douillet">
                <div
                    class="absolute inset-0 bg-black bg-opacity-70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out flex items-center justify-center">
                    <div
                        class="absolute top-1/2 left-28 bg-opacity-70 p-4 rounded-md text-white whitespace-nowrap space-y-2 z-10 transform -translate-y-1/2">
                        <h3 class="text-3xl font-bold"> {{ __('content.nidDouillet') }} </h3>
                        <p class="text-lg font-normal">2 {{ __('content.pax') }} | 60m² | 8m
                            {{ __('content.hauteur') }}</p>
                    </div>
                </div>
            </a>
            ...
            <a href="{{ route('cabane2') }}" class="relative group block overflow-visible mr-32">
                <img class="h-[200px] w-[250px] object-cover transition-opacity duration-300 ease-in-out group-hover:opacity-60"
                    src="{{ Storage::url('images/osmose.png') }}" alt="Cabane Osmose">
                <div
                    class="absolute inset-0 bg-black bg-opacity-70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out flex items-center justify-center">
                    <div
                        class="absolute top-1/2 left-28 bg-opacity-70 p-4 rounded-md text-white whitespace-nowrap space-y-2 z-10 transform -translate-y-1/2">
            ...
            <a href="{{ route('cabane3') }}" class="relative group block overflow-visible ml-32">
                <img class="h-[200px] w-[250px] object-cover transition-opacity duration-300 ease-in-out group-hover:opacity-60"
                    src="{{ Storage::url('images/escapade.png') }}" alt="Cabane Escapade">
                <div
                    class="absolute inset-0 bg-black bg-opacity-70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out flex items-center justify-center">
                    <div
                        class="absolute top-1/2 left-28 bg-opacity-70 p-4 rounded-md text-white whitespace-nowrap space-y-2 z-10 transform -translate-y-1/2">
                        <h3 class="text-3xl font-bold">{{ __('content.escapade') }} </h3>
                        <p class="text-lg font-normal">4 {{ __('content.pax') }} | 85m² + {{ __('content.ilot') }} |
                            8m {{ __('content.hauteur') }} </p>
                    </div>
                </div>
            </a>
            ...
            <a href="{{ route('cabane4') }}" class="relative group block overflow-visible mr-32">
                <img class="h-[200px] w-[250px] object-cover transition-opacity duration-300 ease-in-out group-hover:opacity-60"
                    src="{{ Storage::url('images/eden.png') }}" alt="Cabane Eden">
                <div
                    class="absolute inset-0 bg-black bg-opacity-70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out flex items-center justify-center">
                    <div
                        class="absolute top-1/2 left-28 bg-opacity-70 p-4 rounded-md text-white whitespace-nowrap space-y-2 z-10 transform -translate-y-1/2">
                        <h3 class="text-3xl font-bold">{{ __('content.eden') }} </h3>
                        <p class="text-lg font-normal">6 {{ __('content.pax') }} | 110m² | 6m
                            {{ __('content.hauteur') }} </p>
                    </div>
                </div>
            </a>
    ...
    <div style="background-color:#F9F4EE" class="w-full h-screen">
        ...
        <div class="flex flex-col items-center justify-center">
            <span class="font-bold text-custom-marron text-[40px] relative top-6 uppercase"> {{ __('content.plan') }}
            </span>
